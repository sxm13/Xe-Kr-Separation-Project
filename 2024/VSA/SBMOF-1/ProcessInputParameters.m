...
%% State all input parameters for the simulation
    ...
    y_0        = 0.2                   ;   % Inlet gas CO2 mole fraction[-]
    Ctot_0     = P_0/R/T_0              ;   % Inlet total concentration [mol/m^3]
    v_0        = ndot_0/Ctot_0          ;   % Inlet velocity and scaling parameter [m/s]
    mu         = 2.11e-5                ;   % Viscosity of gas [Pa*s]
    epsilon    = 0.343                  ;   % Void fraction
    D_m        = 0.57e-5              ;   % Molecular diffusivity [m^2/s]
    K_z        = 0.00565                   ;   % Thermal conduction in gas phase [W/m/k]
    C_pg       = 20.7                   ;   % Specific heat of gas [J/mol/k]
    C_pa       = 20.8                   ;   % Specific heat of adsorbed phase [J/mol/k]
    MW_CO2     = 0.1312                ;   % Molecular weight of CO2 [kg/mol]
    MW_N2      = 0.083798                ;   % Molecular weight of N2 [kg/mol]
    ...
    ro_s        = 1570.42              ;   % Density of the adsorbent [kg/m^3]
    %ro_s       = material_propertry(1) ;
    r_p         = 5e-3                  ;   % Radius of the pellets [m]
    C_ps        = 881.202               ;   % Specific heat capacity of the adsorbent [J/kg/K]
    q_s         = 4.85                  ;   % Molar loading scaling factor [mol/kg]
    ...
    % Isotherm parameters
    q_s_b      = [1.4611,  1.4800]   ;   % Saturation loading on site b [mol/kg]
    q_s_d      = [0,  0]   ;   % Saturation loading on site d [mol/kg]
    b          = [5.3424e-09,  1.5948e-10]   ;   % Pre-exponential factor for site b [Pa-1]
    d          = [1,  1]  ;   % Pre-exponential factor for site d [Pa-1]
    deltaU_b   = [-30034.56703,  -30017.9997]  ;   % Heat of adsorption for site b [J/mol]
    deltaU_d   = [0,  0]  ;   % Heat of adsorption for site d [J/mol]
    deltaU     = [-36178.5273, -26338.8861]     ; 
